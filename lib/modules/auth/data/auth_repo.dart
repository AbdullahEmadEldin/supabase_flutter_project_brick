...
import 'package:mizan/core/di/di.dart';
import 'package:mizan/core/networking/failure_model.dart';
import 'package:mizan/core/networking/resolve_async.dart';
import 'package:mizan/modules/auth/data/user_model.dart';
...
class AuthRepo {
  ...
  Future<Either<Failure, UserModel>> registerUser(UserModel newUser) async {
    return await resolveSupabaseAsync(() async {
      final user = await getIt<SupabaseClient>().auth.signUp(
        email: newUser.email!,
  ...
  Future<Either<Failure, UserModel>> signIn(
    ...
    return await resolveSupabaseAsync(() async {
      final user = await getIt<SupabaseClient>().auth.signInWithPassword(
        email: email,
        password: password,
      );
      // fetch profile with subcriptions
  ...
  Future<Either<Failure, UserModel>> uploadDeviceId(String deviceId) async {
    return await resolveSupabaseAsync(() async {
      final user = await getIt<SupabaseClient>().auth.updateUser(
        UserAttributes(data: {'device_id': deviceId}),
      );
      //update device id in profile table also
